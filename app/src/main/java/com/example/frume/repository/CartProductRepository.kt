            ...
            return try {
                selectedListDocId.forEach { cartProductDocId ->
                    try {
                        ...
                        Log.d("test100", "CartProductRepository -> deleteCartProducts() 삭제 성공 ID: $cartProductDocId")
                    } catch (e: Exception) {
                        Log.e("test100", "CartProductRepository -> deleteCartProducts() 삭제 실패 ID: $cartProductDocId", e)
                    }
                }
                true // 모든 삭제 작업이 완료되면 true 반환
            } catch (e: Exception) {
                Log.e("test100", "CartProductRepository -> deleteCartProducts() 전체 삭제 중 오류 발생", e)
                false // 전체 삭제 실패 시 false 반환
            }
        }
